...
const HeroText: React.FC<HeroTextProps> = ({ badge, title, description }) => (
  <div className="grid max-xl:place-items-center">
    ...
    <h1 className="block mt-1 text-4xl font-extrabold tracking-tight sm:text-5xl xl:text-6xl">
      <span className="block text-gray-900">{title.primary}</span>
      <span className="block text-primary-600">{title.accent}</span>
    </h1>
...
const HeroActions: React.FC<HeroActionsProps> = ({
  ...
  <div className="mt-8 sm:max-w-lg sm:mx-auto sm:text-center xl:text-left xl:mx-0">
    <div className="flex flex-col space-y-4 sm:flex-row sm:space-y-0 sm:space-x-4">
      ...
      >
        {secondaryIcon && <Icon name={secondaryIcon} className="w-8 h-6 mr-2" />}
        {secondaryLabel}
      </Link>
    </div>
    {note && <p className="mt-3 text-sm text-center text-gray-500 xl:text-left">{note}</p>}
  </div>
);
...
const HeroDashboardPreview: React.FC<HeroDashboardPreviewProps> = ({
  streak,
  weeklyGoal,
  workout,
}) => (
        ...
        <div className="p-4 bg-linear-to-br from-primary-50 to-white">
          <div className="grid grid-cols-2 gap-4 mb-4">
            ...
            <div className="p-3 bg-white rounded-lg shadow-sm">
              ...
              <div className="text-lg font-bold text-gray-900">{weeklyGoal}</div>
            </div>
          </div>
...
const HeroContainer: React.FC = () => {
  return (
    <section className="grid mx-auto max-w-7xl xl:grid-cols-2 my-25 xl:my-40">
      <div>
        <HeroText
        ...
        <HeroActions
          primaryLabel={HERO_CONTENT.cta.primary.label}
          secondaryLabel={HERO_CONTENT.cta.secondary.label}
          secondaryIcon={HERO_CONTENT.cta.secondary.icon as IconName}
          note={HERO_CONTENT.cta.note}
        />
      </div>
